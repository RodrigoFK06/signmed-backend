...
async def startup_event():
    ...
    logger.info(f"📁 Directorio de uploads creado/verificado: {upload_dir.absolute()}")
    
    # Montar directorio de uploads como archivos estáticos
    uploads_root = Path("uploads")
    if uploads_root.exists():
        app.mount("/uploads", StaticFiles(directory=str(uploads_root)), name="uploads")
        logger.info(f"📂 Archivos estáticos montados en /uploads desde {uploads_root.absolute()}")
